...
int main()
{
    ...
    auto step3_end = std::chrono::system_clock::now();
    download_synchronous_in_threads();
    ...
    std::cout << "Synchronous: " << get_sec(start, step1_end) << " s." << std::endl
        ...
        << "Multiplexing: " << get_sec(step2_end, step3_end) << " s." << std::endl
        << "Threads: " << get_sec(step3_end, end) << " s." << std::endl;
}